...
class Graph{
    ...
    void path_counter(int src, int des, vector<bool>& vertics, int& path_count)
    ...
    int path_counter(int src, int des, int v);

    Graph::Graph(int v){
        m_vertics = v;
        m_neighbours = new list<int>[v];

    }

    void Graph::addEdge(int src, int des){
        m_neighbours[src].push_back(des);
    }

    int Graph::count_paths(int src, int des, int vertics){
        int path_count = 0;
        vector<bool> visited(vertics, false)
        path_counter(src, dis, visited, vertics);
        return count_paths;
    }

    void Graph::path_counter(int src, int des, vector<bool>& visited, int& path_count){
        visited[src] = true;
        if(src==des){
            path_count++;
        }
        else{
            for(auto neighbr: m_neighbours[src]){
                if(!visited[neighbr]){
                    path_counter(neighbr, des, visited, path_count);
                }
            }
        }
        visited[src] = false;
